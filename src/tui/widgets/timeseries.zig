...
pub const TimeSeriesChart = struct {
    ...
    pub fn render(self: TimeSeriesChart, buf: *Buffer, area: Rect) void {
        ...
        for (self.timestamps, self.values) |timestamp, value| {
            // Map to screen coordinates with bounds clamping
                ...
                screen_y >= plot_area.y and screen_y < plot_area.y + plot_area.height)
            {
                ...
                if (prev_screen_x != null and prev_screen_y != null) {
                    ...
                    if (dx == 0 and dy == 0) {
                        ...
                        while (line_y <= end_y) : (line_y += 1) {
                            buf.set(px, line_y, .{ .char = '│', .style = self.line_style });
                        }
                    } else if (dy == 0) {
                        // Horizontal line
                        ...
                        while (line_x <= end_x) : (line_x += 1) {
                            buf.set(line_x, py, .{ .char = '─', .style = self.line_style });
                        }
                    } else {
                        ...
                        while (i <= steps) : (i += 1) {
                            ...
                            {
                                buf.set(interp_x, interp_y, .{ .char = '─', .style = self.line_style });
                            }
                        }
                    }
                }
                ...
                // Draw point marker
                if (self.show_points) {
                    buf.set(screen_x, screen_y, .{ .char = self.point_char, .style = self.line_style });
                }
