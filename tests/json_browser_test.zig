...
test "render — string value uses string_style" {
    ...
    // cursor = nodes.len → no node is selected, so value style (not cursor_style) is used
    var b = JsonBrowser{ .nodes = &nodes, .cursor = nodes.len };
...
test "render — non-cursor node does not use cursor_style" {
    ...
    try testing.expect(buf.get(0, 1).?.style.fg == null or buf.get(0, 1).?.style.fg.? != Color.red);
}
...
test "render — sibling after collapsed object is visible" {
    ...
    // Row 0: '{'
    try testing.expectEqual(@as(u21, '{'), buf.get(0, 0).?.char);
    // Row 1: "  a: { ... }" — "  " (2) + "a" (1) + ":" (1) + " " (1) = col 5 for '{'
    try testing.expectEqual(@as(u21, '{'), buf.get(5, 1).?.char); // indent(2) + "a: " = col 5
    // Row 2: "  b: 99" — sibling (hidden nodes 2&3 skipped); 'b' at col 2
